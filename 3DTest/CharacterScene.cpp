...
void CharacterScene::HandleInput(float dt) {
	...
	// --- tonemap A/B, live (same controls as ConstraintScene). Cycling the curve carries its own
	// matched exposure, because the five operators differ by up to ~2.7x in GAIN -- at one fixed
	// exposure you compare brightness rather than curve shape, and the brighter curve always loses
	// for the wrong reason. ACESFitted 0.35 was tuned by eye; the rest are solved back from it.
...
void CharacterScene::Draw() {
	...
	// is fixed, so a smaller box means more texels per meter and crisper contact shadows. 22 units
	...
	// doubles the texels per meter. 12 units around the player covers the surrounding columns and
	...
	char tone[160];
	sprintf_s(tone, "tonemap %s   exposure %.2f   [T cycles curve, [ / ] exposure]",
	          kToneNames[(unsigned)r3d.GetTonemapper() % (unsigned)_countof(kToneNames)], r3d.GetExposure());
	r2d.SubmitText(*font, 10.0f, 80.0f, tone, 0.85f, JLib::Colors::Gray);
}
